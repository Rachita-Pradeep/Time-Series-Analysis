...
#define NSTREAM 3 		// no of concurrnt c
...
void buf_init(buffer_array_t buf) {
...
/// enqueue the circular 
void buf_enqueue(buffer_array_t buf, buffer_candidate_t cand){
...
buffer_candidate_t buf_dequeue(buffer_array_t buf){
...
{
	// this is a initialization kernel used to initialize the shared memory
	// the query is added to the shared memory to make memory access more efficient
...
int main(int argc, char *argv[]) {
	...
	cudaStream_t stream_array [NSTREAM] ; // declaring cuda streams - used concurrent kernel
	...
	/// initialize the kim-keogh buffer queue
	buffer_array_t kim_cir_buf;

	/// initialize the keogh-dtw queue
	buffer_array_t kim_cir_buf;
	...
	while (!done) {
		...
		if (it == 0) {
			// initialization of the kernel - shared memory - done only once
			initKernel<<<N, M, m * sizeof(double)>>>(d_q, m);
			
			// cuda streams
			for ( int is = 0 ; is<NSTREAM; is++)
				cudaStreamCreate(&stream_array[is]) ;
			epoch = 100000;
			while (ep < epoch) {
		...
		if (ep <= m - 1) {
			...
			// call kim kernel for ith chunk
			kim_kernel<<<N/3 ,M/3 , , stream_array[0] >>>();

			// call keogh kernel for i chunk
			keogh_kernel<<<N/3 ,M/3 , , stream_array[1] >>>();


			// call dtw for i chunk
			dtw_kernel<<<N/3 ,M/3 , , stream_array[2] >>>();
	...
	for(int is = 0 ; is<NSTREAM; is++)
		cudaStreamDestroy(stream_array[is]);
	fclose(fp);
