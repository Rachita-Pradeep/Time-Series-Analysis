...
int parse_read_done = 0;
int parse_buffer_index = 0;
int parse_n = 0;
int parse_temp_index = 0;
char parse_buf[4096];
char parse_temp[14];
int parse_main_flag = 1;
...
float parse_nread(){
	// int flag = 0;
	while(1){
		if(parse_buffer_index == parse_n){
			parse_n = (int)fread(parse_buf, 1, 4096, fp);
			parse_buffer_index = 0;
			if(parse_n == 0){
				parse_read_done = 1;
				return atof(parse_temp);
			}
			continue;
		}else if(parse_buf[parse_buffer_index] == '\n'){
			parse_buffer_index++;
			parse_temp_index = 0;
			return atof(parse_temp);
		}else{
			parse_temp[parse_temp_index++] = parse_buf[parse_buffer_index++];
			continue;
		}
	}
}
...
//		printf("Kernel done.\n");
			...
			time_spent = (double) (end - begin) / CLOCKS_PER_SEC;
		printf("Time taken for reading %lf \n", time_spent);
